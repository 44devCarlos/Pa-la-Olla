...
const CommentsSection = ({ recipeId, initialComments, onCommentPosted }) => {
    ...
    const handleSubmitComment = async () => {
        ...
        if (!userString) {
            Alert.alert("Inicia Sesión", "Debes iniciar sesión para dejar un comentario.");
            return;
        }
        if (!commentText.trim()) {
            Alert.alert("Comentario Vacío", "Por favor, escribe un comentario.");
            return;
        }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.commentForm}>
                ...
                />
                {/* Aquí podrías añadir un componente de estrellas para seleccionar el rating */}
            ...
            {initialComments.length > 0 ? (
                ...
                initialComments.map((comment, index) => (
                    <View key={comment.id_comentario || index.toString()} style={styles.comment}>
                        <Text style={styles.commentName}>{comment.nombre_usuario}</Text>
                        <Text style={styles.commentRating}>{comment.calificacion} ⭐</Text>
                        <Text style={styles.commentDate}>{new Date(comment.fecha_comentario).toLocaleDateString()}</Text>
                        <Text style={styles.commentText}>{comment.descripcion}</Text>
                    </View>
                ))
            ) : (
...
const styles = StyleSheet.create({
    ...
    subtitle: { marginTop: 24, fontSize: 20, fontWeight: '700', color: '#7f1d1d', marginBottom: 16 },
    commentForm: { backgroundColor: '#fff', padding: 16, borderRadius: 10, elevation: 2 },
    formTitle: { fontSize: 16, fontWeight: '600', marginBottom: 8 },
    input: { borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 10, marginBottom: 12, height: 80, textAlignVertical: 'top' },
    submitButton: { backgroundColor: '#F21B4E', padding: 12, borderRadius: 6, alignItems: 'center' },
    submitButtonText: { color: 'white', fontWeight: '600' },
    comment: { backgroundColor: '#fff', borderRadius: 10, padding: 12, marginTop: 12, elevation: 2 },
