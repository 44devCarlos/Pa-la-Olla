...
const RecipeInfo = ({ receta, ratings }) => {
	const avgRating = parseFloat(ratings[0]?.calificacion_promedio).toFixed(1);

	return (
		<View>
			<Image
				source={{ uri: receta.imagen_receta }}
				style={styles.image}
			/>
			<View style={styles.content}>
				...
				<View style={styles.metricsContainer}>
					<Metric
						icon="clock"
						label="Tiempo"
						value={receta.rango_tiempo}
					/>
					<Metric
						icon="users"
						label="Porciones"
						value={receta.rango_porciones}
					/>
					<Metric
						icon="star"
						label="Rating"
